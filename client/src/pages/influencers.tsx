...
import { useDate } from "@/lib/date-context"; // AJOUT : Import du contexte Date
  ...
  return (
    <div className="space-y-4 p-4 border rounded-md bg-muted/30">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <Input
          type="number"
          placeholder={t("inf.form.followers")}
...
function InfluencerFormDialog({
    ...
    if (open) {
      ...
      if (influencer?.whatsapp) {
        ...
        if (foundCode) {
          ...
          setPhoneNumber(influencer.whatsapp);
        }
      } else {
        setPhonePrefix("33");
        setPhoneNumber("");
  ...
  const onSubmit = (data: InfluencerFormData) => {
    let finalWhatsapp = "";
    if (phoneNumber) {
      finalWhatsapp = `${phonePrefix}${phoneNumber.replace(/^0+/, "")}`;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {influencer ? t("inf.form.edit_title") : t("inf.form.add_title")}
          </DialogTitle>
            ...
            <div className="flex items-start gap-4">
              <div className="flex flex-col items-center">
                ...
                >
                  {form.watch("profileImageUrl") ? (
                    <img src={form.watch("profileImageUrl")} alt="Profile" className="h-full w-full object-cover" />
                  ) : (
                    <div className="text-center p-2">
                      <Upload className="h-6 w-6 mx-auto mb-1 text-muted-foreground" />
                      <span className="text-[10px] text-muted-foreground">{t("inf.form.upload")}</span>
                    </div>
                  )}
                </div>
                <input
              ...
              <div className="flex-1 space-y-3">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("inf.form.name")}</FormLabel>
                      <FormControl><Input placeholder="Sarah Mode" {...field} data-testid="input-name" /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                <div className="space-y-2">
                  <FormLabel>{t("inf.form.whatsapp")}</FormLabel>
                  <div className="flex gap-2">
                    ...
                    <Input placeholder="612345678" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value.replace(/[^0-9]/g, ''))} className="flex-1" />
                  </div>
                </div>
              </div>
            </div>
            {/* Autres champs (Gender, Email, Notes...) - Identiques */}
...
export default function Influencers() {
  ...
  const { data: influencers, isLoading } = useQuery<InfluencerWithStats[]>({
    queryKey: ["/api/influencers/stats", from, to], // Clé dynamique
    queryFn: async () => {
      const res = await fetch(`/api/influencers/stats?from=${from}&to=${to}`);
      return res.json();
    }
  });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-semibold" data-testid="text-page-title">{t("inf.title")}</h1>
      ...
      {isLoading ? (
        ...
        <Card className="p-12">
          <div className="text-center">
            ...
            <h3 className="text-lg font-medium mb-2">{t("inf.no_influencers")}</h3>
